...
async def execute(clustering_methods_list, clustering_details_list, cluster_names_list,
    ...
    # TODO Get labeled data here using a separate method
    # TODO Make this in another class
    labeled_data_col_name = "Labeled Data"
    # Getting the labeled data
    test1_sheet = pd.read_excel(file_name, sheet_name="Test 1")

    # The labeled data column
    labeled_data_column = test1_sheet[labeled_data_col_name][1:]

    assert labeled_data_column is not None

    labeled_data_df = pd.DataFrame(labeled_data_column)

    # Renames columns of all read data for consistency
    labeled_data_df = labeled_data_df.rename(columns={labeled_data_col_name: "Data"})

    # Makes all read data numeric
    labeled_data_df["Data"] = pd.to_numeric(labeled_data_df["Data"], downcast="float")
    ...
    # Visualize the labeled clustered data
    dah.perform_analysis(data_df=data_df, x_df=x_df, y_df=y_df,
                         clustered_data=labeled_data_df,
                         prop=clustered_column, show_contour_clustered=show_contour_clustered,
                         show_contour_raw=show_contour_raw, show_bar=show_bar,
                         cluster_name="Labeled Data", cluster_iter=0,
                         save_clustered_contour=save_contour_clustered,
                         give_cluster_report=give_cluster_report, print_to_console=print_to_console,
                         print_to_text_file=print_to_text_file, text_file_name=text_file_path,
                         clusters_save_dir=cluster_save_path, save_cluster_histograms=save_cluster_histograms,
    ...
    while i < len(clustering_methods_list):
        ...
        # TODO Change from a triangle to a single for loop which compares to the labeled data
        clustered_df1 = list_of_clustered_data[i]
        clustered_df2 = labeled_data_df
        ...
        cluster_pair = str(cluster_names_list[i]) + " and Labeled Data"
        rand_index_queue.put((rand_index, cluster_pair))

        curr_iter_save_path = rand_index_vis_path + "/RandIndexVis_" + str(i) + "_" \
            + str(cluster_names_list[i]) + ".png"
        ...
        cch.rand_index_visualization(x_df=x_df, y_df=y_df, clustered_df1=clustered_df1,
                                     ...
                                     cluster1_name=cluster_names_list[i], cluster2_name="Labeled Data",
                                     cluster1_iter=i, cluster2_iter=0)
        i = i + 1
